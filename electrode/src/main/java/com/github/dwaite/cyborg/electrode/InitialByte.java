...
public final class InitialByte implements Comparable<InitialByte> {
	...
	public enum LogicalType {
		...
		private static LogicalType fromInitialByte(Major major, InfoFormat format, int infoValue) {
			switch(major) {
			case INTEGER:
	...
	static {
		...
		for (int i = 0; i < 256; i++) {
			...
			if (format == InfoFormat.INDEFINITE && !major.isIndefiniteAdditionalInfoAllowed()) {
				continue;
			}
	...
	private InitialByte(byte initialByteValue) {
		...
		this.logicalType = LogicalType.fromInitialByte(major, format, InfoFormat.maskBits(initialByteValue));
	}
	...
	public String toString() {
		return "InitialByte(code=" + Byte.toUnsignedInt(byteValue) + ", major=" + major + ", format=" + format + ")"; 
	}
}